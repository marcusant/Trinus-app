...
6. **Regra de Consulta de Documentos**: Para qualquer nova feature, consulte obrigatoriamente a pasta `docs/` para se inteirar do `ARCHITECTURE.md`, `FLOWS.md`, e `DATA_MODEL.md`.
7. **Fluxo de Trabalho Obrigatório**: SEMPRE valide e teste as alterações localmente (`npm run build` ou testes manuais) antes de considerar a tarefa concluída. NÃO realize `git commit` ou `git push` sem que o utilizador peça explicitamente.
...
13. **Consulta ao Oráculo ECC**: Sempre que o agente (IA) se deparar com uma refatoração complexa, desafio de segurança, arquitetura de banco (Supabase/PostgreSQL) ou padrões avançados de Frontend, o agente DEVE e ESTÁ AUTORIZADO a consultar silenciosamente os agentes e skills contidos no diretório de rascunho (`C:\Users\Marcus\.gemini\antigravity-ide\scratch\ECC`) em busca de padrões e soluções de estado da arte antes de implementar código duvidoso.
